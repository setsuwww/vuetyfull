<template>
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="emit('close')">
    <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
      <h2 class="text-lg font-bold text-gray-800 mb-4">Add a Review</h2>
  ...
  if (rating.value > 0 && comment.value.trim()) {
    try {
      const response = await axios.post(
        `http://localhost:4321/api/destinations/${destinationId}/comments`,
        {
    ...
    console.warn('Please provide a rating and comment');
  }
}
</script>
